...
# Display labels for the monosaccharide axis: the anomeric configuration is not
# recorded in S2_Table, so it is supplied here from the residue identity.  Applied
# only when rendering tick labels — the underlying data keys stay the bare
# abbreviations, so grouping and ordering are unaffected.  Residues missing from
# the map fall back to their bare abbreviation.
MONO_DISPLAY_LABELS = {
    "Glc": "D-Glc",
    "GlcA": "D-GlcA",
    "Gal": "D-Gal",
...
def _mono_label(mono: str) -> str:
    """Return the configuration-prefixed display label for a monosaccharide."""
    return MONO_DISPLAY_LABELS.get(str(mono).strip(), str(mono))
...
class KTypePlotAPI:
    ...
    def _render_modification_distribution_chart(
        ...
        ax_mono.set_xticklabels(
            [_mono_label(m) for m in data.monos], rotation=45, ha="right"
        )
