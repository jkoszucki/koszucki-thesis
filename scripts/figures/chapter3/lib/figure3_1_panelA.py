"""
Figure 3.1, Panel A — best SGNH hydrolase predictors per K-locus (precision ≥ 0.50).
...
Outputs:
    plots_dir/figure3_1-panelA.png / .pdf
    plots_dir/legends/figure3_1-panelA-legend.png / .pdf
"""
...
def plot_figure3_1_panelA(
    ...
    """
    Produce Figure 3.1 Panel A: horizontal dot plot of best SGNH predictor per K-locus.
    ...
    plt.tight_layout()
    for ext in ("png", "pdf"):
        out = plots_dir / f"figure3_1-panelA.{ext}"
        fig.savefig(out, dpi=dpi, bbox_inches="tight")
        print(f"  [figure3_1-panelA] → {out.name}")
    plt.close(fig)
    ...
    plt.tight_layout()
    for ext in ("png", "pdf"):
        out = legends_dir / f"figure3_1-panelA-legend.{ext}"
        fig_leg.savefig(out, dpi=dpi, bbox_inches="tight")
        print(f"  [figure3_1-panelA-legend] → legends/{out.name}")
    plt.close(fig_leg)
