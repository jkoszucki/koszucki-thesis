"""
Figure 4.2, Panel B — SGNH hydrolase similarity network (Cytoscape).
...
Outputs (Cytoscape import files):
    plots_dir/figure4_2-panelB/node.tsv
    plots_dir/figure4_2-panelB/edge.tsv
...
def plot_figure4_2_panelB(
    ...
    Args:
        ...
        plots_dir:          output directory; files go into plots_dir/figure4_2-panelB/
        style:              cfg.style (optional, unused but kept for API consistency)
    """
    out_dir  = plots_dir / "figure4_2-panelB"
    ...
    if (node_out.exists() and node_out.stat().st_size > 0 and
        ...
        print("  figure4_2-panelB/node.tsv + edge.tsv exist — skipping (delete to recompute)")
        return
    ...
    print(f"  → figure4_2-panelB/node.tsv ({len(nodes)} rows)")
    print(f"  → figure4_2-panelB/edge.tsv ({len(edges)} rows)")
