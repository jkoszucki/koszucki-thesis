"""
Figure 4.3, Panel A — GWAS prediction and acetylation overview table.

Table with 35 most-abundant K-loci as columns and four evidence rows, ordered so
the acetylation story reads as a block: the phage enzyme that removes the
modification, the capsule enzyme that installs it, the modification itself, and
the unrelated depolymerase evidence last (author's ordering, 2026-09-03):

  Row 1: Predicted prophage deacetylases (SGNH best per locus, 12 K-loci)
          strong (precision >= 0.80) → sgnh_domain_color; likely → light gold
  Row 2: K-locus acetyltransferase candidates (acetylases_kloci.tsv, 59 K-loci)
          present → sslbh_color (blue)
  Row 3: Acetylation reported in CPS repeating unit (modifications sheet)
...
The acetyltransferase and acetylation rows were a single merged row with
left/right subcells until 2026-08-25; they are now separate full-width rows.
The row order above supersedes the capsule-first ordering used until 2026-09-03,
and the acetylation row label was shortened from "O-acetylation" to
"Acetylation" on the same date.

All filled cells show a count. Cells without data are white.

Outputs:
    plots_dir/figure4_3-panelA.png / .pdf
"""
...
# every part of figure 4.3 — see figureS11_panelA._EXCLUDED_PROTEINS.
...
def _ktype_to_locus(ktype: str) -> str | None:
    """Convert a K-type to its K-locus: 'K2' -> 'KL2', 'K21b' -> 'KL21'.

    Serological subtypes resolve to the locus they belong to, so a modification
    on either subtype is attributed to that locus (author decision, 2026-09-01;
    this reverses the 2026-07-29 convention that excluded subtyped structures).
    Returns None for types with no locus at all, such as 'UNK'.
    """
    s = ktype.strip()
    if not s.startswith("K"):
        return None
    rest = s[1:].rstrip("abcdefghijklmnopqrstuvwxyz")
    if not rest.isdigit():
        return None
    return f"KL{rest}"
...
def _load_o_acetylation(cps_xlsx: Path) -> dict[str, tuple[str, int]]:
    ...
    A locus counts as acetylated if O-acetylation was identified on **any** of its
    structures, by any method — K-PAM or the primary literature alike (author
    decision, 2026-09-01). Where a locus has several structures and only some
    are acetylated (KL2, KL21), the locus still counts as acetylated.

    This replaces a filter that scored KPAM-sourced structures only; it had
    dropped K2's acetylation (recorded on the LIT structure CPS78_K2) and, via
    _ktype_to_locus, KL21's (recorded on the subtyped CPS22_K21b).
    """
    mods = pd.read_excel(cps_xlsx, sheet_name="capsule_modifications")
    oac = mods[mods["modification"] == "acetylation"].copy()
...
def plot_figure4_3_panelA(
    ...
    """
    Produce Figure 4.3 Panel A: GWAS prediction overview table.
    ...
    for ext in ("png", "pdf"):
        out = plots_dir / f"figure4_3-panelA.{ext}"
        fig.savefig(out, dpi=dpi, bbox_inches="tight")
        print(f"  [figure4_3-panelA] → {out.name}")
    plt.close(fig)
    ...
    columns = [
        [
            _header("Deacetylase prediction"),
        ...
        [
            _header("Depolymerase prediction"),
    ...
    plt.tight_layout()
    for ext in ("png", "pdf"):
        out = legends_dir / f"figure4_3-panelA-legend.{ext}"
        fig_leg.savefig(out, dpi=dpi, bbox_inches="tight")
        print(f"  [figure4_3-panelA-legend] → legends/{out.name}")
    plt.close(fig_leg)
